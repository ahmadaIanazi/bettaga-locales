...
export default function PassActionStamps({ handleAction, item }) {
  ...
  /** UI */
  const HEADER_LABEL_CURRENT = 'Current Stamps: ';
  const HEADER_LABEL_NEW = 'Updated Stamps: ';
  ...
  const GET_LEVEL = Math.floor(
    Math.min(Math.max(Math.floor((GET_VALUE / MAX_VALUE) * 100) / 10 + 1, 0), LEVELS)
  );
  const GET_HEADER_VALUE = `${GET_VALUE}/${MAX_VALUE}`;
...
const styles = StyleSheet.create({
  headingWrap:{ justifyContent: 'center', alignItems: 'center', flexDirection: 'row', marginBottom: 20},
  inputWrap: {
